...
def prepare_features(load_data: pd.DataFrame, lookback_days: int = 90) -> pd.DataFrame:
    ...
    df["temperature"] = (
        70
        + 20 * np.sin(2 * np.pi * df["day_of_year"] / 365.25)
...
sample_data = pd.DataFrame(
    {
        ...
        "mw": np.random.normal(25000, 5000, 24 * 90),
        "ba": "CAL-ALL",
    }
)
...
def train_arima_model(df: pd.DataFrame, ba: str) -> str | None:
    ...
    with mlflow.start_run(run_name=f"arima_{ba}"):
        ...
        model_info = mlflow.sklearn.log_model(
            model, "model", registered_model_name=f"leap_{ba}_arima"
        )
...
def train_lightgbm_model(df: pd.DataFrame, ba: str) -> str | None:
    ...
    with mlflow.start_run(run_name=f"lightgbm_{ba}"):
        ...
        model_info = mlflow.sklearn.log_model(
            model, "model", registered_model_name=f"leap_{ba}_lightgbm"
        )
...
for scenario in scenarios:
    scenario_features = np.where(
        scenario == "baseline",
        sample_features.copy(),
...
class OutageImpactAnalyzer:
    ...
    def get_outage_adjustment_factor(self, state: str, timestamp: datetime) -> float:
        ...
        total_customers = sum(
            r.get("total_customers", 0) for r in outage_data if r.get("total_customers")
        )
    ...
    def adjust_forecast_for_outages(
